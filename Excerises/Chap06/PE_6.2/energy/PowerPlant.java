...
class Control {
  ...
  public void runSystem() {
    // TT implementation provided here:
    // Continuously monitor optimal throughput
    for(;;) {
      // If the throughputs differ by more than 10 units, i.e. tolerance, adjust the reactor throughput
      int currentOptimalThroughput = thePlant.getOptimalThroughput();
      // Check if adjustment is needed
      try {
        boolean plantNeedsAdjustment = needAdjustment(currentOptimalThroughput);
        if (plantNeedsAdjustment)
          adjustThroughput(currentOptimalThroughput);
      }
  ...
  public boolean needAdjustment(int target) throws ReactorCritical {
    // TT implementation provided here:
    // Report if the reactor goes critical
    if ( thePlant.core.isCritical() )
      throw new ReactorCritical();
    // Reports whether the throughput of the reactor needs adjusting
    int currentThroughput = thePlant.core.getThroughput();
    if ( Math.abs(currentThroughput - target)  > TOLERANCE )
      return true;
    else
      return false;
  }
  ...
  public void adjustThroughput(int target) throws ReactorCritical {
    // TT implementation provided here:
    int currentThroughput = thePlant.core.getThroughput();
    // Above tolerance, need to lower throughput
    if ( (currentThroughput - target)  > TOLERANCE ) {
      thePlant.core.decreaseThroughput();
    }
    // Below tolerance, need to raise throughput
    else if ( (target - currentThroughput)  > TOLERANCE ) {
  ...
  public void shutdown() {
    // TT implementation provided here:
    int currentThroughput = thePlant.core.getThroughput();
    while (currentThroughput > 0) {
      thePlant.core.decreaseThroughput();
      currentThroughput = thePlant.core.getThroughput();
    }
  }
}